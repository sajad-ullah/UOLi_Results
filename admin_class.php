...
public function save_students(){
	...
	// Basic validation
	if(empty($student_code) || empty($firstname) || empty($lastname) || empty($gender) || empty($class_id) || empty($department_id)){
		return 0; // missing required fields
	}

	// Check for duplicate student_code (excluding current record)
	$check = $this->db->prepare("SELECT id FROM students WHERE student_code = ? " . (isset($id) && $id > 0 ? "AND id != ?" : ""));
	if(isset($id) && $id > 0){
		$check->bind_param("si", $student_code, $id);
	} else {
	...
	$check->store_result();
	if($check->num_rows > 0){
		return 2; // duplicate student_code
	}
	$check->close();
	...
	if(isset($id) && $id > 0){
		...
		$stmt = $this->db->prepare("INSERT INTO students 
			(student_code, firstname, middlename, lastname, gender, address, class_id, department_id, date_created) 
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())");
